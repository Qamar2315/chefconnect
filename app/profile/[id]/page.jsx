      ...
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <div className="mt-8">
          ...
          {session && session.user.user_id === userData._id && (
            <Link href={`/profile/${session.user.user_id}/edit`} className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded mt-10">
              Edit Profile
            </Link>
          )}
          {userRecipes.length > 0 ? (
            <div className="container mx-auto px-4 py-8">
              <h1 className="text-3xl font-bold mb-8 text-white">{userData.name} Recipes</h1>
                  ...
                  <div key={recipe._id}>
                    ...
                    </Link>
                    {
                      userData?._id === session?.user?.user_id && (
                        <button onClick={() => { deleteRecipe(recipe) }} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded mt-2">
                          Delete Recipe
                        </button>
                      )
                    }
